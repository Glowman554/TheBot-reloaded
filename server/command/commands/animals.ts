...
export default class Animals implements loadable {
	load(): void {
		...
		command_manager.add_command(new Command("dog", "Cuteness awaits!", "Use '#dog' to see a cute dog!", {
			execute: async (event: CommandEvent): Promise<CommandResponse> => {
				if (event.interface.args.length != 0) {
					return fail;
				}
		...
		command_manager.add_command(new Command("cat", "Cuteness awaits!", "Use '#cat' to see a cute cat!", {
			execute: async (event: CommandEvent): Promise<CommandResponse> => {
				if (event.interface.args.length != 0) {
					return fail;
				}
